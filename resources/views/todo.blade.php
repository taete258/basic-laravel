        ...
        <x-jet-custome-modal id="create-todo-modal">
            ...
            <x-slot name="content">
                <div>
                    ...
                    <x-jet-input id="toDoName" name="name" type="text" class="mt-1 block w-full" autofocus
                        autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" />
                    <span class="text-red-500 mt-2" id="toDoNameError"></span>
                </div>
                <div class="mt-3">
                    <x-jet-label for="description" value="{{ __('Description') }}" />
                    <x-jet-textarea id="description" name="description" type="text" class="mt-1 block w-full h-[70px]"
                ...
                <div class="mt-3 task-input-group">
                    ...
                    <a href="#" class="float-right add_form_field text-gray-700 text-sm hover:text-indigo-500"
                        type="button" onclick="addMoreTask(event)">Add New Field &nbsp;
        ...
        return $('#dataTable').dataTable({
            ...
            columns: [{
                ...
                },
                {
                    data: "state",
                    render: function(data, type, rowData, meta) {
                        return '<span class="' + (rowData.state == 'Archived' ? 'bg-green-600' :
                                'bg-red-600') +
                            ' text-white text-md inline-flex items-center justify-center px-3 py-1 leading-none rounded-full">' +
                            rowData.state + '</span>';
                    }
                },
                {
    ...
    function createToDo(e) {
        ...
        $('.taskName input[name="tasksData"]').each(function() {
            if ($(this).val() != '') {
                data.push({
                    name: $(this).val(),
                    description: ($(this).nextAll('[name="tasksDescription"]').val() != '') ? $(this)
                        .nextAll('[name="tasksDescription"]').val() : null
                });
            }
        });
        let formData = {
